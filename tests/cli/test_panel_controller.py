...
def test_add_router_rejects_deterministic_password_env_collisions_before_secret_write(
    tmp_path,
    monkeypatch,
) -> None:
    prompts = ScriptedPromptAdapter(
    ...
    assert f"Password env '{password_env}' is already used by router 'home-router'" in str(
        exc_info.value
    )
    assert controller._config_path.read_text(encoding="utf-8") == original_config
