...
- **Managed-only:** меняются только object-groups, DNS route bindings и static routes из managed mappings, плюс явно настроенный managed `default_route`.
...
- **TLS/SAN boundary:** transport выдаёт typed diagnostic всех DNS endpoint с SNI, SAN-only matching (без CN fallback) и metadata сертификата. Обычный transport всегда verified; отдельный unverified ACME transport имеет только три фиксированные операции и создаётся после panel confirmation.
...
CLI dry-run/sync
  ...
  -> for each enabled router:
       -> resolve secret
       -> create RCI client
       -> read default-route interface state when routers[].default_route.managed=true
       -> read object-groups, DNS routes, static routes and DNS proxy state
       -> compute/apply default-route priority diff before service mappings
